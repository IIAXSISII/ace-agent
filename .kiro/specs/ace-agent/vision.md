...
- **Jaeger**: An open-source distributed tracing system used locally to visualize agent execution traces across Sub-Agent hops.
...
13. IN local development, Jaeger SHALL receive all ADOT traces and display the full end-to-end trace including Chainlit → Orchestrator → Sub-Agent → Gateway hops with all span attributes, enabling multi-hop debugging.
...
16. IN local development, Grafana SHALL provide pre-built dashboards pre-wired to Jaeger, Langfuse, and VictoriaMetrics. Grafana is a local-only tool run via `docker-compose` and is NOT deployed to AWS.
17. THE repository SHALL include a `docker-compose.yml` at the repo root that starts a Local_Observability_Stack consisting of: a Chainlit UI (port 8000), an ADOT Collector, Langfuse, Jaeger, VictoriaMetrics, and Grafana — all pre-configured to receive telemetry from locally running agents. Standalone Prometheus is NOT included — VictoriaMetrics handles all metrics scraping and storage.
...
20. A developer SHALL be able to start the full Local_Observability_Stack with a single `docker compose up` command and immediately see agent traces in Jaeger, LLM traces in Langfuse, and metrics in Grafana without any additional configuration.
